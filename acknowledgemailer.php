<?php
		$testrun=isset($_RESPONSE['test'])? $_RESPONSE['test'] : 'no';
       //echo "arrived at mailer";
	   $from_name=isset($_POST['pubname'])? $_POST['pubname'] : "Newsletter publisher";
	   $event_title= isset($_POST['title']) ? $_POST['title'] : "Untitled Event";
		...
		$recipient = isset($_POST['recipient']) ? $_POST['recipient'] : "cauleyfj@yahoo.com";
        
		if($testrun == 'yes')
		{
			$recipient ="cauleyfj@yahoo.com";
		}
        // Set the email subject.
        $subject = "Your recent event information submission";

        // Build the email content.
		$email_content = sprintf("From:  %s ,\n\n",$from_name);
		...
		echo('the recipient is '.$recipient);
		echo("<br>The subject is". $subject);
		echo("<br>Content is ".$email_content);
